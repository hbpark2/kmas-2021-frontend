...
import { CREATE_SUCCESS_TEXT, DELETE_SUCCESS_TEXT, MODIFY_SUCCESS_TEXT } from "../../constants";
...
const MarketFormContainer = () => {
	...
	const {
		...
		({ id, formData }: { id: number; formData: FormData }) => PUT_MARKET(id, formData),
		{
			onSuccess,
		}
	);
	...
	const {
		...
		({ id, formData }: { id: number; formData: FormData }) => DELETE_MARKET(id, formData),
		{
			onSuccess,
		}
	);
	...
	const loading = isPostMarketLoading || isPutMarketLoading || isMarketDataLoading;
