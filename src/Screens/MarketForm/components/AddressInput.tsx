...
const AddressInput: React.FC<AddressInpuProps> = ({ onPostOpen }) => {
	...
	return (
		<label htmlFor="detail_address">
			<span>주소</span>
			<div>
				<input type="text" id="zonecode" readOnly={true} {...register("zonecode")} />
				<button onClick={(e) => onPostOpen(e)} type="button">
					우편번호 찾기
				</button>
			</div>
			<div>
				<input type="text" id="jibun_address" readOnly={true} {...register("jibun_address")} />
			</div>
			<div style={{ display: "none" }}>
				<input type="text" id="road_address" readOnly={true} {...register("road_address")} />
			</div>
